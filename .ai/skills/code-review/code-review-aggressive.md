...
1. Run or inspect deterministic context first: `scripts.checks.review_scope_check`, `validate-code`, and any relevant docs/repo aggregates.
2. Build a review map of changed behavior areas and touched subsystems from the classifier and diff.
3. Run standard defect checks for regressions, semantic contract drift, and migration safety.
4. Run required negative-path checks:
   - invalid inputs
   - not-found and empty data handling
   - rollback or compatibility behavior
   - permission or safety guard regressions
5. Run required simplification checks:
   - stale compatibility paths
   - dead or redundant helpers
   - duplicate tests and fixtures
   - obsolete docs wording
6. For each stale-code or obsolescence claim, record evidence from reference searches, public API/route/type usage, tests covering old and new paths, and migration or backward-compatibility risk.
7. Produce a test adequacy matrix for each changed behavior area:
   - existing tests
   - missing tests
   - commands run and outcome
8. If no hard defects are found, provide zero-findings evidence.
...
- Do not duplicate deterministic check findings; reference the failed command in Validation instead.
