...
Assembles the chain-linked out-of-sample series from the experiment's persisted
...
from trading.backtesting.optimizer_models import ChainLinkedOOSSeries, ChainLinkSegment
...
def fetch_chain_linked_oos(conn: sqlite3.Connection, *, experiment_id: int) -> ChainLinkedOOSSeries | None:
    """Return the experiment's chain-linked OOS series, or ``None`` if unavailable.
    ...
    segments: list[ChainLinkSegment] = []
    ...
    return chain_link_oos(segments)
