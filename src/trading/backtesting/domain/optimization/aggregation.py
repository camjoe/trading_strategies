"""Chain-linked out-of-sample aggregation for a walk-forward optimization run.
...
independently reset account, so window returns are **compounded** (chain-linked),
never summed on equity. A window whose OOS interval does not immediately follow
the previous one — a ``step_months`` longer than ``test_months`` — is flagged so
the discontinuity is visible rather than silently smoothed over.
...
from trading.backtesting.domain.metrics import PERCENT_SCALE
from trading.backtesting.optimizer_models import (
    ChainLinkedOOSPoint,
    ChainLinkedOOSSeries,
    ChainLinkSegment,
)
...
def period_return_pct(*, first_equity: float, last_equity: float) -> float:
    ...
    Matches the standalone backtest report's total-return definition, so a chain-link
...
def chain_link_oos(segments: list[ChainLinkSegment]) -> ChainLinkedOOSSeries:
    """Compound non-overlapping per-window OOS returns into a chronological series.

    Returns are multiplied (chain-linked), never summed on equity, because each OOS
    ...
    return ChainLinkedOOSSeries(
        points=points,
        chain_linked_return_pct=(growth - 1.0) * PERCENT_SCALE,
