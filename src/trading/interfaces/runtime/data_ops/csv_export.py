...
def zip_export_directory(export_dir: Path) -> Path:
    ...
    archive_path = Path(
        shutil.make_archive(str(archive_base), "zip", root_dir=str(resolved_dir.parent), base_dir=resolved_dir.name)
    )
    return archive_path
...
def print_export_summary(result: ExportBatchResult) -> None:
    print(f"[export] Database: {result.db_path}")
    print(f"[export] Output:   {result.output_dir}")
    for table_result in result.tables:
        print(f"[export] {table_result.table}: {table_result.row_count} row(s) -> {table_result.output_path.name}")
