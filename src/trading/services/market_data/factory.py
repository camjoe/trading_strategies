"""Pure builders for market-data and feature providers.

These functions resolve the configured provider (constructor argument, then
``TRADING_MARKET_DATA_PROVIDER`` env var, then the ``local/market_data_config.json``
file, then the default) and return a *fresh* instance each call. They hold no
module-level state — composition roots call ``build_provider`` /
``build_feature_provider`` once at entry and inject the result down the call
chain (mirroring the broker factory pattern).
"""
