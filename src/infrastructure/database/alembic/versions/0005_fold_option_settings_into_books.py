"""Fold option settings into books; drop the 1:1 table and account columns.

Revision ID: 0005
Revises: 0004

Option/leaps settings are book-owned (database-cleanup-roadmap item A3,
decided 2026-07-16): the eleven option knobs become columns on books,
replacing both the book_option_settings 1:1 table and the legacy account
option columns that option selection still read. Backfill order per book:
its book_option_settings row when present, else the parent account's values,
else NULL — so every book's effective settings are unchanged.

Self-contained by convention: no application imports, literal DDL only.
Column adds use ALTER TABLE ADD COLUMN; the column drops are explicit table
rebuilds.
"""
...
# Option columns added to books — DDL mirrors the book_option_settings table
# from revision 0001.
_BOOK_OPTION_COLUMN_DDL = (
...
# The 0004 books shape (base + execution columns), used by the downgrade
# rebuild that removes the option columns again.
_BOOK_0004_COLUMNS = (
    "id",
    "account_id",
    "name",
...
_ACCOUNTS_DDL_TEMPLATE = """
    CREATE TABLE accounts_new (
        ...
        goal_period TEXT NOT NULL DEFAULT 'monthly',{option_block}
...
def _check_foreign_keys() -> None:
    ...
    if orphans:
        raise RuntimeError(f"revision 0005 rebuild left FK violations: {orphans!r}")


def upgrade() -> None:
    for ddl in _BOOK_OPTION_COLUMN_DDL:
        op.execute(f"ALTER TABLE books ADD COLUMN {ddl}")

    # Backfill 1: from the book's own settings row where one exists.
    assignments = ", ".join(
        f"{column} = (SELECT s.{column} FROM book_option_settings s WHERE s.book_id = books.id)"
        for column in _OPTION_COLUMNS
    )
    op.execute(
        f"UPDATE books SET {assignments} "
        "WHERE EXISTS (SELECT 1 FROM book_option_settings s WHERE s.book_id = books.id)"
    )
    # Backfill 2: books without a settings row inherit the parent account's
    # legacy values.
    account_assignments = ", ".join(
        f"{column} = (SELECT a.{column} FROM accounts a WHERE a.id = books.account_id)" for column in _OPTION_COLUMNS
    )
    op.execute(
        f"UPDATE books SET {account_assignments} "
        "WHERE NOT EXISTS (SELECT 1 FROM book_option_settings s WHERE s.book_id = books.id)"
    )

    op.execute("DROP TABLE book_option_settings")

    # Drop the legacy option columns from accounts (rebuild).
    column_list = ", ".join(_KEPT_ACCOUNT_COLUMNS)
    op.execute(_ACCOUNTS_DDL_TEMPLATE.format(option_block=""))
...
def downgrade() -> None:
    ...
    base_list = ", ".join(_BOOK_0004_COLUMNS)
    op.execute(_BOOKS_0004_DDL)
    ...
    # Restore the account option columns; values return as DDL defaults
    # (the pre-migration backup is the data recovery path).
    kept_list = ", ".join(_KEPT_ACCOUNT_COLUMNS)
    op.execute(_ACCOUNTS_DDL_TEMPLATE.format(option_block=_ACCOUNTS_OPTION_BLOCK))
