...
def run_term_definitions_check(repo_root: Path, registry_rel_path: str = "docs/reference/finance.json") -> int:
    ...
    glossary_terms = parse_glossary(repo_root / "docs/reference/Finance.md")
    ...
    for term_name in sorted(registry_terms):
        ...
        if use in {"both", "glossary"}:
            if glossary_entry is None:
                drift_issues.append(f"- {term_name}: missing from Finance")
            else:
                if glossary_entry[1] != definition:
                    drift_issues.append(f"- {term_name}: Finance definition differs from registry")
                if glossary_entry[0] != group:
                    drift_issues.append(f"- {term_name}: Finance section differs from registry group")
        elif glossary_entry is not None:
            drift_issues.append(f"- {term_name}: unexpectedly present in Finance")
    ...
    print(f"Finance terms parsed: {len(glossary_term_names)}")
    ...
    if not missing_from_registry and not extra_in_glossary and not extra_in_ui and not drift_issues:
        print("\nPASS: term registry is in sync with Finance/UI source docs.")
        return 0
    ...
    if extra_in_glossary:
        print(f"- Terms present in Finance but missing from registry: {', '.join(extra_in_glossary[:10])}")
    if extra_in_ui:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate docs/reference/finance.json is synchronized with Finance and UI docs.",
    )
    parser.add_argument(
        "--repo-root",
        default=None,
    ...
    parser.add_argument(
        "--registry",
        default="docs/reference/finance.json",
