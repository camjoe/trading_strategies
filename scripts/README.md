...
- `sqlite.py`: inspects any SQLite database file and writes neutral schema JSON, standalone HTML, or both.
- `render_html.py`: renders a neutral schema JSON payload to standalone HTML.
- `sqlite_introspection.py`: builds the neutral payload from a SQLite connection.
- `html_viewer.py`: contains the project-agnostic HTML renderer used by all adapters.
