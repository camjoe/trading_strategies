...
def test_fetch_report_raises_for_missing_run(conn) -> None:
    with pytest.raises(ValueError, match="not found"):
        report_service.fetch_report(conn, run_id=999)
...
def test_fetch_report_raises_when_no_snapshots(conn) -> None:
    ...
    with pytest.raises(ValueError, match="No snapshots"):
        report_service.fetch_report(conn, run_id=run_id)


def test_latest_run_reads_return_none_for_an_account_with_no_runs(conn) -> None:
    create_backtest_account(conn, "acct_no_runs")

    assert report_service.fetch_latest_run_for_account(conn, account_name="acct_no_runs") is None
    assert report_service.fetch_latest_run_id_for_account(conn, account_name="acct_no_runs") is None
...
def test_latest_and_recent_backtest_run_wrappers_map_repository_rows(conn) -> None:
    ...
    latest = report_service.fetch_latest_run_for_account(conn, account_name="acct_wrappers")
    recent = report_service.fetch_recent_runs(conn, limit=2)
    latest_id = report_service.fetch_latest_run_id_for_account(conn, account_name="acct_wrappers")
...
def test_report_summary_splits_the_stored_warnings_column(conn) -> None:
    ...
    report = report_service.fetch_report(conn, run_id=run_id)
