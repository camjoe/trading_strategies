"""P3 clean trading-unit schema: tables, invariants, and FK enforcement.
...
NEW_TABLES = {
    ...
    "unit_option_settings",
    "unit_rotation_settings",
...
def _insert_account(conn, name: str = "acct_units") -> int:
    cursor = conn.execute(
...
def _insert_unit(conn, account_id: int, name: str = "default", is_default: int = 1) -> int:
    cursor = conn.execute(
        """
...
def test_one_default_unit_per_account_enforced(conn) -> None:
    account_id = _insert_account(conn)
    _insert_unit(conn, account_id, name="default", is_default=1)
    # A second non-default unit is fine.
    _insert_unit(conn, account_id, name="second", is_default=0)

    with pytest.raises(sqlite3.IntegrityError):
        _insert_unit(conn, account_id, name="third", is_default=1)


def test_one_open_assignment_per_unit_enforced(conn) -> None:
    account_id = _insert_account(conn)
    unit_id = _insert_unit(conn, account_id)
        ...
        conn.execute(
            ...
            INSERT INTO unit_strategy_assignments (
                ...
                created_at, updated_at
                      ...
                      '2026-07-03T00:00:00Z', '2026-07-03T00:00:00Z')
            """,
            (unit_id, strategy_id, effective_to),
        )
...
def test_foreign_keys_are_enforced(conn) -> None:
    with pytest.raises(sqlite3.IntegrityError):
        _insert_unit(conn, account_id=999_999)


def test_deleting_account_cascades_to_units_and_settings(conn) -> None:
    account_id = _insert_account(conn)
    unit_id = _insert_unit(conn, account_id)
    conn.execute(
        """
        INSERT INTO unit_execution_settings (unit_id, created_at, updated_at)
    ...
    assert conn.execute("SELECT COUNT(*) FROM trading_units").fetchone()[0] == 0
    assert conn.execute("SELECT COUNT(*) FROM unit_execution_settings").fetchone()[0] == 0
