"""Chain-linked OOS aggregation (pure domain math): compounding, gap detection,
...
from trading.backtesting.domain.optimization.aggregation import chain_link_oos, period_return_pct
from trading.backtesting.optimizer_models import ChainLinkSegment


def _segment(index: int, start: date, end: date, ret: float) -> ChainLinkSegment:
    return ChainLinkSegment(window_index=index, test_start=start, test_end=end, return_pct=ret)
...
class TestChainLink:
    ...
    def test_negative_windows_compound_down(self) -> None:
        ...
        # 0.9 * 0.9 - 1 = -19%
        assert series.chain_linked_return_pct == pytest.approx(-19.0)
    ...
    def test_single_window_series(self) -> None:
        series = chain_link_oos([_segment(1, date(2023, 1, 1), date(2023, 1, 31), 3.5)])
        assert series.chain_linked_return_pct == pytest.approx(3.5)
    ...
    def test_empty_series(self) -> None:
        ...
        assert series.chain_linked_return_pct == pytest.approx(0.0)
        assert not series.has_gaps
