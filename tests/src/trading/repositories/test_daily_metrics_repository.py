...
class TestFetchRecentReturnsForBook:
    def _upsert_return(self, conn, *, account_id, book_id, metric_date, return_pct) -> None:
    ...
    def test_returns_only_dates_before_the_boundary(self, conn) -> None:
        acct_id = _account_id(conn)
        bk_id = _book_id(conn, acct_id)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date="2026-01-01", return_pct=1.0)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date="2026-01-02", return_pct=2.0)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date="2026-01-03", return_pct=3.0)
    ...
    def test_most_recent_first_and_limited(self, conn) -> None:
        ...
        for day, ret in (("2026-01-01", 1.0), ("2026-01-02", 2.0), ("2026-01-03", 3.0), ("2026-01-04", 4.0)):
            self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date=day, return_pct=ret)
        returns = DailyMetricsRepository(conn).fetch_recent_returns_for_book(
    ...
    def test_excludes_null_returns(self, conn) -> None:
        acct_id = _account_id(conn)
        bk_id = _book_id(conn, acct_id)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date="2026-01-01", return_pct=None)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_id, metric_date="2026-01-02", return_pct=2.0)
        returns = DailyMetricsRepository(conn).fetch_recent_returns_for_book(
            book_id=bk_id, before_date="2026-01-03", limit=10
        )
    ...
    def test_isolated_per_book(self, conn) -> None:
        ...
        self._upsert_return(conn, account_id=acct_id, book_id=bk_a, metric_date="2026-01-01", return_pct=1.0)
        self._upsert_return(conn, account_id=acct_id, book_id=bk_b, metric_date="2026-01-01", return_pct=9.0)
        returns = DailyMetricsRepository(conn).fetch_recent_returns_for_book(
