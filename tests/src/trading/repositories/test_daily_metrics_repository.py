...
def _upsert(conn, *, account_id: int, sleeve_id: int | None, metric_date: str, return_pct: float = 1.0) -> int:
...
class TestUpsert:
    def test_insert_returns_id(self, conn) -> None:
        ...
        row_id = _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01")
        assert row_id > 0

    def test_second_upsert_returns_same_id(self, conn) -> None:
        acct_id = _account_id(conn)
        id_a = _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01")
        id_b = _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01", return_pct=2.0)
        assert id_a == id_b

    def test_upsert_updates_existing_values(self, conn) -> None:
        acct_id = _account_id(conn)
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01", return_pct=1.0)
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01", return_pct=9.9)
    ...
    def test_sleeve_and_portfolio_rows_are_distinct(self, conn) -> None:
        ...
        id_sleeve = _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-01")
        id_portfolio = _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01")
...
class TestFetchForAccount:
    ...
    def test_only_returns_rows_for_requested_account(self, conn) -> None:
        ...
        _upsert(conn, account_id=acct_a, sleeve_id=None, metric_date="2026-01-01", return_pct=1.0)
        _upsert(conn, account_id=acct_b, sleeve_id=None, metric_date="2026-01-01", return_pct=2.0)
    ...
    def test_limit_is_respected(self, conn) -> None:
        ...
        for day in ("2026-01-01", "2026-01-02", "2026-01-03"):
            _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date=day)
    ...
    def test_ordered_by_metric_date_desc(self, conn) -> None:
        acct_id = _account_id(conn)
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01")
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-03")
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-02")
        rows = DailyMetricsRepository(conn).fetch_for_account(account_id=acct_id, limit=10)
...
class TestFetchForSleeve:
    def test_empty_when_no_sleeve_metrics(self, conn) -> None:
        acct_id = _account_id(conn)
        slv_id = _sleeve_id(conn, acct_id)
        assert DailyMetricsRepository(conn).fetch_for_sleeve(sleeve_id=slv_id, limit=10) == []

    def test_excludes_portfolio_level_rows(self, conn) -> None:
        acct_id = _account_id(conn)
        slv_id = _sleeve_id(conn, acct_id)
        _upsert(conn, account_id=acct_id, sleeve_id=None, metric_date="2026-01-01")
        _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-01", return_pct=3.0)
        rows = DailyMetricsRepository(conn).fetch_for_sleeve(sleeve_id=slv_id, limit=10)
        assert len(rows) == 1
...
class TestFetchForSleeveWindow:
    def test_returns_only_rows_within_window(self, conn) -> None:
        acct_id = _account_id(conn)
        slv_id = _sleeve_id(conn, acct_id)
        _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-01")
        _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-05")
        _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-10")
        rows = DailyMetricsRepository(conn).fetch_for_sleeve_window(
            sleeve_id=slv_id, start_date="2026-01-03", end_date="2026-01-07"
        )
        assert len(rows) == 1
    ...
    def test_window_boundaries_are_inclusive(self, conn) -> None:
        ...
        _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date="2026-01-05")
        rows = DailyMetricsRepository(conn).fetch_for_sleeve_window(
            sleeve_id=slv_id, start_date="2026-01-01", end_date="2026-01-05"
        )
        assert len(rows) == 2
    ...
    def test_ordered_by_metric_date_asc(self, conn) -> None:
        ...
        for day in ("2026-01-03", "2026-01-01", "2026-01-02"):
            _upsert(conn, account_id=acct_id, sleeve_id=slv_id, metric_date=day)
        rows = DailyMetricsRepository(conn).fetch_for_sleeve_window(
            sleeve_id=slv_id, start_date="2026-01-01", end_date="2026-01-03"
        )
        dates = [r.metric_date for r in rows]
