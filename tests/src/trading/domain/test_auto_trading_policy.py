...
def test_choose_qty_helpers() -> None:
    assert choose_buy_qty(cash=10.0, price=11.0, fee=0.0) == 0
    assert choose_buy_qty(cash=100.0, price=10.0, fee=0.0) == 1
...
def test_estimate_helpers_boundaries() -> None:
    assert estimate_delta(0.0) == pytest.approx(0.55)
    assert estimate_delta(1000.0) == pytest.approx(0.05)
...
def test_option_candidate_allowed_with_delta_and_iv_filters() -> None:
    ...
    ok, _, _ = option_candidate_allowed(
        account,
        "AAPL",
        {"AAPL": 50.0},
    )
    ...
    ok, _, iv_rank = option_candidate_allowed(
        account,
        "AAPL",
        {},
    )
    ...
    ok, delta, iv_rank = option_candidate_allowed(
        account,
        "AAPL",
        {"AAPL": 50.0},
    )
...
def test_apply_leaps_buy_qty_limits() -> None:
    ...
    qty = apply_leaps_buy_qty_limits(qty=5, option_price=100.0, account=account)
    assert qty == 2
...
def test_allocate_buy_quantities_is_independent_of_request_order() -> None:
    ...
    forward = allocate_buy_quantities(requests, cash=700.0, fee_per_trade=1.0)
    reverse = allocate_buy_quantities(list(reversed(requests)), cash=700.0, fee_per_trade=1.0)
...
def test_allocate_buy_quantities_never_commits_more_than_available_cash() -> None:
    ...
    fee = 1.5
    cash = 500.0
    granted = allocate_buy_quantities(requests, cash=cash, fee_per_trade=fee)
...
def test_allocate_buy_quantities_returns_nothing_for_an_empty_bar() -> None:
    assert allocate_buy_quantities([], cash=1000.0, fee_per_trade=0.0) == {}
...
def test_order_signal_candidates_is_deterministic_for_a_seed() -> None:
    ...
    first = order_signal_candidates(candidates, seed="2026-07-30")
    second = order_signal_candidates(candidates, seed="2026-07-30")
    assert first == second
...
def test_order_signal_candidates_ignores_the_order_it_was_given() -> None:
    ...
    candidates = ["AAPL", "MSFT", "NVDA", "AMZN"]
    forward = order_signal_candidates(candidates, seed="2026-07-30")
    reverse = order_signal_candidates(list(reversed(candidates)), seed="2026-07-30")
    assert forward == reverse
...
def test_order_signal_candidates_handles_an_empty_list() -> None:
    assert order_signal_candidates([], seed="2026-07-30") == []


def test_order_capacity_claimants_is_deterministic_for_a_seed() -> None:
    book_ids = [3, 1, 4, 2]
    first = order_capacity_claimants(book_ids, seed="2026-07-30")
    second = order_capacity_claimants(book_ids, seed="2026-07-30")
    assert first == second
...
def test_order_capacity_claimants_ignores_the_order_it_was_given() -> None:
    ...
    book_ids = [1, 2, 3, 4]
    forward = order_capacity_claimants(book_ids, seed="2026-07-30")
    reverse = order_capacity_claimants(list(reversed(book_ids)), seed="2026-07-30")
    assert forward == reverse
...
def test_order_capacity_claimants_does_not_share_an_order_with_tickers() -> None:
    ...
    seed = "2026-07-30"
    assert order_capacity_claimants([1, 2, 3, 4], seed=seed) != [
        int(t) for t in order_signal_candidates(["1", "2", "3", "4"], seed=seed)
    ]


def test_order_capacity_claimants_handles_an_empty_list() -> None:
    assert order_capacity_claimants([], seed="2026-07-30") == []
