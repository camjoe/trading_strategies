...
from trading.services.auto_trading.runtime_sleeve_risk import compute_current_exposure_snapshot
...
def test_compute_current_exposure_snapshot_uses_injected_symbol_sector_map() -> None:
    ...
    sleeves = [SimpleNamespace(current_equity=1_000.0)]

    gross, net, max_symbol_pct, max_sector_pct = compute_current_exposure_snapshot(
        object(),
        account_id=1,
        fetch_sleeve_positions_for_account_fn=lambda *_args, **_kwargs: positions,
        fetch_strategy_sleeves_for_account_fn=lambda *_args, **_kwargs: sleeves,
