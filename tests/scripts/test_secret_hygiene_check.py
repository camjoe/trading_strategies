...
def test_text_secret_assignment_is_reported(tmp_path: Path) -> None:
    path = write_file(tmp_path / ".github/workflows/example.yml", "token: abc123456789\n")
...
def test_env_example_is_not_discovered(tmp_path: Path) -> None:
    write_file(tmp_path / ".env.example", "API_KEY=your-api-key\n")
    write_file(tmp_path / "src/example.py", "from __future__ import annotations\n")
