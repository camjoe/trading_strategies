...
def test_force_run_bypasses_duplicate_guard(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(f"{DAILY_PAPER_TRADING_MODULE}.load_all_account_names", lambda: ["acct_a"])
...
def test_unknown_account_returns_1(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setattr(f"{DAILY_PAPER_TRADING_MODULE}.load_all_account_names", lambda: ["real_acct"])
...
def test_no_accounts_returns_1(monkeypatch, tmp_path: Path, capsys) -> None:
    monkeypatch.setattr(f"{DAILY_PAPER_TRADING_MODULE}.load_all_account_names", lambda: [])
