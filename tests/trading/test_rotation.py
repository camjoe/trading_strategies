...
class TestRotationEdgeAndFailurePaths:
    ...
    def test_parse_rotation_schedule_rejects_unsupported_input_type(self) -> None:
        with pytest.raises(ValueError, match="must be a list or JSON string"):
            parse_rotation_schedule(123)

    def test_parse_rotation_schedule_rejects_invalid_json(self) -> None:
        with pytest.raises(ValueError, match="must be valid JSON"):
            parse_rotation_schedule("[")

    def test_parse_rotation_schedule_blank_string_returns_empty_list(self) -> None:
        assert parse_rotation_schedule("   ") == []

    def test_resolve_active_strategy_falls_back_without_schedule(self) -> None:
        account = {"strategy": "trend", "rotation_schedule": None, "rotation_active_strategy": ""}

        assert resolve_active_strategy(account) == "trend"

    def test_resolve_active_strategy_uses_index_modulo_schedule(self) -> None:
        account = {
            "strategy": "trend",
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
    ...
    def test_is_rotation_due_raises_on_invalid_as_of_iso(self) -> None:
        account = {
            "rotation_enabled": 1,
            "rotation_interval_days": 7,
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
        ...
        with pytest.raises(ValueError, match="as_of_iso must be a valid ISO datetime"):
            is_rotation_due(account, as_of_iso="not-an-iso")

    def test_is_rotation_due_returns_true_when_last_rotation_unset(self) -> None:
        account = {
            "rotation_enabled": 1,
            "rotation_interval_days": 7,
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
    ...
    def test_next_rotation_state_with_short_schedule_uses_resolved_active(self) -> None:
        account = {
            "strategy": "trend",
...
class TestRotationTimeAndGuardrails:
    ...
    def test_is_rotation_due_returns_false_when_not_enabled(self) -> None:
        account = {
            "rotation_enabled": 0,
            "rotation_interval_days": 7,
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
    ...
    def test_is_rotation_due_returns_false_for_short_schedule(self) -> None:
        account = {
            "rotation_enabled": 1,
            "rotation_interval_days": 7,
            "rotation_schedule": dump_rotation_schedule(["trend"]),
    ...
    def test_is_rotation_due_returns_false_when_interval_missing_or_non_positive(self) -> None:
        missing_interval = {
            "rotation_enabled": 1,
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
            "rotation_last_at": "2026-03-01T00:00:00Z",
    ...
    def test_next_rotation_state_uses_now_when_as_of_iso_invalid(self, monkeypatch: pytest.MonkeyPatch) -> None:
        account = {
            "rotation_schedule": dump_rotation_schedule(["trend", "mean_reversion"]),
            "rotation_active_index": 0,
            "rotation_last_at": "2026-02-01T00:00:00Z",
