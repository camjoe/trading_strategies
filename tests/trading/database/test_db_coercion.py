...
class TestPrimitiveCoercion:
    ...
    def test_coerce_str_converts_supported_values(self, raw: object, expected: str) -> None:
        assert db_coercion.coerce_str(raw) == expected

    def test_expect_str_rejects_none_with_field_name(self) -> None:
    ...
    def test_coerce_float_accepts_numeric_like_values(self, raw: object, expected: float) -> None:
        assert db_coercion.coerce_float(raw) == expected

    def test_coerce_float_rejects_non_convertible_type(self) -> None:
    ...
    def test_coerce_bool_accepts_common_representations(self, raw: object, expected: bool) -> None:
        assert db_coercion.coerce_bool(raw) is expected

    def test_coerce_bool_rejects_unrecognized_string(self) -> None:
        with pytest.raises(ValueError, match="Invalid boolean value"):
            db_coercion.coerce_bool("maybe")

    def test_expect_int_rejects_none_with_field_name(self) -> None:
        with pytest.raises(ValueError, match="count cannot be null"):
            db_coercion.expect_int(None, "count")


class TestRowHelpers:
    def test_row_helpers_coerce_and_expect(self, sample_row: sqlite3.Row) -> None:
    ...
    def test_row_expect_helpers_reject_null_values(self, sample_row: sqlite3.Row) -> None:
        with pytest.raises(ValueError, match="missing cannot be null"):
            db_coercion.row_expect_str(sample_row, "missing")


class TestObjectAdapters:
    def test_to_float_obj_delegates_to_expect_float(self) -> None:
        assert db_coercion.to_float_obj("2.5") == 2.5

    def test_to_int_obj_delegates_to_expect_int(self) -> None:
