...
class TestSchemaInitialization:
    def test_ensure_db_creates_core_tables(self, sqlite_backend: SQLiteBackend) -> None:
        conn = db.ensure_db()
        try:
...
class TestColumnMigrationHelpers:
    def test_ensure_column_applies_post_sql_for_new_column(self, sqlite_backend: SQLiteBackend) -> None:
        conn = sqlite_backend.open_connection()
        try:
            conn.executescript(
                """
    ...
    def test_ensure_column_is_noop_when_column_exists(self, sqlite_backend: SQLiteBackend) -> None:
        conn = sqlite_backend.open_connection()
        try:
            conn.executescript(
                """
