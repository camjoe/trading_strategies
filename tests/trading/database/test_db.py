...
def test_init_schema_migrates_legacy_test_account_to_canonical_manual_only(
    sqlite_backend: SQLiteBackend,
) -> None:
    conn = sqlite_backend.open_connection()
    try:
        ...
        row = conn.execute(
            "SELECT name, account_kind, live_trading_enabled FROM accounts WHERE name = 'test_account'"
        ).fetchone()
        assert row is not None
        ...
        legacy = conn.execute(
            "SELECT 1 FROM accounts WHERE name = 'test_account_bt'"
        ).fetchone()
...
def test_init_schema_merges_legacy_and_canonical_test_accounts(
    sqlite_backend: SQLiteBackend,
) -> None:
    conn = sqlite_backend.open_connection()
    try:
        conn.executescript(
            ...
            INSERT INTO accounts (name, account_kind, strategy, initial_cash, created_at, live_trading_enabled)
            VALUES ('test_account', 'manual_only', 'trend', 1000, '2026-01-01T00:00:00Z', 0);

            INSERT INTO accounts (name, account_kind, strategy, initial_cash, created_at, live_trading_enabled)
            VALUES ('test_account_bt', 'test_shadow', 'trend', 1000, '2026-01-01T00:00:00Z', 1);

            INSERT INTO trades (account_id, ticker, side, qty, price, fee, trade_time, note)
            VALUES (2, 'AAPL', 'buy', 1, 100, 0, '2026-01-01T00:00:00Z', 'legacy');
            """
        )
        ...
        accounts = conn.execute(
            "SELECT id, name, account_kind, live_trading_enabled FROM accounts WHERE name IN ('test_account', 'test_account_bt') ORDER BY id"
        ).fetchall()
        assert len(accounts) == 1
        assert accounts[0]["name"] == "test_account"
        assert accounts[0]["account_kind"] == "manual_only"
        assert int(accounts[0]["live_trading_enabled"]) == 0

        trade = conn.execute("SELECT account_id FROM trades").fetchone()
        assert trade is not None
        assert int(trade["account_id"]) == int(accounts[0]["id"])
    finally:
        conn.close()
