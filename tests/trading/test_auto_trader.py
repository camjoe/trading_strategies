...
# ---------------------------------------------------------------------------
# Input Loading And Primitive Helpers
# ---------------------------------------------------------------------------

class TestInputLoadingAndPrimitiveHelpers:
    def test_load_tickers_from_file_dedupes_ignores_comments(self, tmp_path):
    ...
    def test_load_tickers_from_file_missing_raises(self, tmp_path):
    ...
    def test_choose_qty_helpers(self, monkeypatch):
    ...
    def test_estimate_helpers_boundaries(self):
...
# ---------------------------------------------------------------------------
# LEAPS Candidate And Risk Filters
# ---------------------------------------------------------------------------

class TestLeapsCandidateAndRiskFilters:
    def test_option_candidate_allowed_with_delta_and_iv_filters(self):
    ...
    def test_apply_leaps_buy_qty_limits(self):
    ...
    def test_build_trade_note_for_leaps_buy(self):
    ...
    def test_choose_side_prioritizes_forced_sell(self, monkeypatch):
    ...
    def test_prepare_buy_trade_equity(self, monkeypatch):
    ...
    def test_prepare_buy_trade_leaps(self, monkeypatch):
    ...
    def test_prepare_sell_trade_leaps_forced_sell(self, monkeypatch):
    ...
    def test_build_iv_rank_proxy_handles_empty_and_single(self, monkeypatch):
...
class TestSelectionAndPreparationHelpers:
    def test_choose_sell_ticker_by_risk_stop_and_target(self, monkeypatch):
    ...
    def test_choose_buy_ticker_learning_and_fallback(self, monkeypatch):
    ...
    def test_choose_sell_ticker_learning(self, monkeypatch):
    ...
    def test_build_leaps_candidates_filters(self, monkeypatch):
        account = _base_account()
    ...
    def test_prepare_buy_trade_returns_none_when_no_candidates(self, monkeypatch):
    ...
    def test_prepare_sell_trade_returns_none_when_invalid_price(self, monkeypatch):
...
class TestTradeLoopOrchestration:
    ...
    def test_prepare_trade_selection_uses_forced_sell_path(self, monkeypatch):
        state = SimpleNamespace(positions={"AAPL": 2.0}, avg_cost={"AAPL": 100.0})
        account = _base_account()

        monkeypatch.setattr(auto_trader, "_choose_side", lambda *_args, **_kwargs: "sell")
        monkeypatch.setattr(auto_trader, "_prepare_sell_trade", lambda *_args, **_kwargs: ("AAPL", 1, 95.0))

        selection = auto_trader._prepare_trade_selection(
            account=account,
...
# ---------------------------------------------------------------------------
# Rotation And Strategy Selection
# ---------------------------------------------------------------------------

class TestRotationAndStrategySelection:
    def test_rotate_account_if_due_updates_state(self, monkeypatch):
    ...
    def test_rotate_account_if_due_optimal_previous_period_best(self, conn) -> None:
    ...
    def test_rotate_account_if_due_optimal_average_return(self, conn) -> None:
    ...
    def test_rotate_account_if_due_noop_when_not_due(self, monkeypatch):
        account = _base_account(rotation_enabled=1, rotation_interval_days=30, rotation_last_at="2026-03-20T00:00:00Z")
        monkeypatch.setattr(auto_trader, "is_rotation_due", lambda *_args, **_kwargs: False)

        out = auto_trader._rotate_account_if_due(conn=object(), account_name="acct", account=account, now_iso="2026-03-21T00:00:00Z")

        assert out is account

    def test_select_optimal_strategy_returns_none_when_no_runs(self, conn):
        account = _base_account(id=123, rotation_schedule='["trend","mean_reversion"]')
        assert auto_trader._select_optimal_strategy(conn, account, "2026-03-21T00:00:00Z") is None

    def test_select_optimal_strategy_returns_none_when_schedule_empty(self, conn):
        account = _base_account(id=123, rotation_schedule="[]")
        assert auto_trader._select_optimal_strategy(conn, account, "2026-03-21T00:00:00Z") is None


class TestInternalHelpers:
    def test_parse_as_of_iso_handles_naive_and_zulu_inputs(self):
        naive = auto_trader._parse_as_of_iso("2026-03-21T12:00:00")
        zulu = auto_trader._parse_as_of_iso("2026-03-21T12:00:00Z")
    ...
    def test_parse_as_of_iso_converts_offset_timezone_to_utc(self):
        parsed = auto_trader._parse_as_of_iso("2026-03-21T12:00:00+02:00")
        assert parsed.isoformat().endswith("+00:00")

    def test_safe_return_pct_returns_percent_for_valid_inputs(self):
        assert auto_trader._safe_return_pct(100.0, 110.0) == pytest.approx(10.0)

    def test_refresh_account_state_delegates_to_load_and_compute(self, monkeypatch):
        ...
        monkeypatch.setattr(auto_trader, "load_trades", _fake_load_trades)
        monkeypatch.setattr(auto_trader, "compute_account_state", _fake_compute)

        out = auto_trader._refresh_account_state(conn=object(), account=account)
        assert out == "STATE"
