...
class TestBacktestsRoutes:
    ...
    def test_backtest_run_endpoint_validation_error_returns_400(
        self,
        api_client: TestClient,
        seed_account: Callable[..., None],
    ) -> None:
        ...
        run_mock = Mock(side_effect=ValidationError("bad config"))
    ...
    def test_backtest_run_endpoint_unexpected_value_error_propagates(
        self,
        api_client: TestClient,
        seed_account: Callable[..., None],
    ) -> None:
        # A bare ValueError is an unexpected server error and is no longer masked
        # as a 400 client error (docs/adr/007-ui-error-mapping.md).
        seed_account("acct_run_bug")

        run_mock = Mock(side_effect=ValueError("unexpected"))
        with patch(_RUN_BACKTEST, run_mock):
