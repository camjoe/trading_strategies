...
def build_sleeve_env(
    conn,
    *,
    account_name: str = "acct_sleeve",
    start_equity: float = 1_000.0,
    snapshot_time: str = "2026-05-03T13:59:00Z",
) -> SimpleNamespace:
    """Create an account + active sleeve + matching snapshot.

    Returns a SimpleNamespace with ``account_name``, ``account_id``,
    ``sleeve_id``.  Used by the ``sleeve_env`` fixture and any test
    that needs a minimal sleeve runtime environment without a rotation
    schedule.
    """
...
def build_rotation_sleeve_env(
    ...
    *,
    ...
    """Create a full rotation-ready sleeve environment.

    Includes account with rotation schedule, sleeve, incumbent strategy
    assignment, two metric rows, and a matching snapshot.  Returns a
    SimpleNamespace with ``account_name``, ``account_id``, ``sleeve_id``.
    """
