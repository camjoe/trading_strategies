...
| `docs/reference/money-representation-plan.md` | Plan to move money and quantity storage from float to integer minor units with fractional-share precision: scope, open decisions, staged order | A decision in the plan is made, or a stage lands, or the change completes and the plan retires |
