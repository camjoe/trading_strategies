...
| Asset | Description |
|---|---|
| `trade_universes/` | Named ticker lists. A **write-time shorthand** only: naming one stores its expansion in `books.trade_symbols`, so nothing reads these on the trading path. `default.txt` doubles as `DEFAULT_TICKERS_FILE` for surfaces that take an explicit path — backtests, strategy lab, benchmark sweeps |
