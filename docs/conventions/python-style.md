...
It applies to all Python under `src/trading/`, `paper_trading_ui/backend/`, `scripts/`, `src/common/`, and `tests/`.
...
```bash
.venv/bin/ruff check src/trading/ paper_trading_ui/backend/ tests/ --select E,W,N --line-length 119 --statistics
```
...
Ruff scan run against `src/trading/`, `paper_trading_ui/backend/`, `tests/` on 2026-05-10
