...
This module reuses account metadata from paper trading while storing run/trade/equity history in dedicated backtest tables.
...
Use monthly universe reconstitution snapshots (`YYYY-MM.txt`):
...
- Signals use prior-day data and execute on subsequent bars to reduce look-ahead bias.
- Daily adjusted close data is used; intraday path is not modeled.
- Stop-loss / take-profit behavior is approximate when evaluated on daily bars.
