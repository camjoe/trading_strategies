...
1. Add new table and index DDL to `trading/database/db_schema.py`.
2. Keep schema changes additive-only.
3. Reuse existing init flow in `trading/database/db_init.py`.
4. Use `ColumnMigration` tuples in `trading/database/db_migrations.py` only for future additive column evolutions on sleeve tables.
