...
from trading.repositories.snapshots import fetch_latest_snapshot_row
...
def reconcile_sleeves_vs_latest_snapshot(
    ...
    *,
    ...
    latest_snapshot = fetch_latest_snapshot_row(conn, account_id=int(account_id))
    if latest_snapshot is None:
        raise ValueError(f"No equity snapshot available for account_id={account_id}.")
    return reconcile_sleeves_vs_account_equity(
        conn,
        account_id=int(account_id),
        account_equity=row_expect_float(latest_snapshot, "equity"),
        tolerance=tolerance,
        snapshot_time=row_expect_str(latest_snapshot, "snapshot_time"),
    )
